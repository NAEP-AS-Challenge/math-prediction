...
There are four “type ii” items which were composed of multiple sub-items or parts that each have their own set of scores and response fields. The other six items we are calling "type i" items. These items also had multiple parts; however, these parts are considered dependently linked portions of the item and, as such, were assigned a single score that encompasses all of the portions as a single overall score. (One of the six "type i" items, VH302907, was designed as a "type ii" multi-part item, but we are treating it as a "type i" single-score item, and only requesting participants predict the "part A" sub-item component.)

While all items, both type i and type ii, have an overall integer score which we've named "assigned_score", as well as a non-integer overall score (named "score") which contains partial credit codes ("1A", "2B"), for one item, VH302907, the "assigned_score" is not the score we would like participants should predict. To make it clear which score contestants should predict, we’ve created a variable “`score_to_predict`”. We’ve also created a variable named “`predict_from`” to identify the most relevant constructed response text to use when creating predicted scores.
...
These data have been parsed to make them easier to process. The parsed XML data, in contrast to the common variables listed above, are different for each item. The item specific variables are described below
the item name in the list that follows. Please note, the format of the data values for the process data (e.g. eliminations) may differ by year for the same item. For example, eliminations may be recorded as “(1, 2, 5)” in 2017 and “1, 2, 5” in 2019.

Also note, the CR text has been parsed but not completely cleaned. The data were analyzed for sensitive information (e.g. personally-identifiable  information, profanity, toxic language) and some responses were removed as a result. However, spellcheck has not been applied to correct what may be obvious spelling errors.
...
| VH271613 |   0.977   |   type ii        |
| VH302907 |   0.980   |   type i         |
| VH304954 |   0.985   |   type i         |
| VH507804 |   0.991   |   type ii        |
| VH525628 |   0.957   |   type i         |
...
|:--------:|:-----:|----------:|:---------:|:------:|--------:|:----------------:|
| VH134067 | 0.966 |         1 |     2     | 4,483  |  40,346 |  type i          |
| VH139380 | 0.981 |         1 |     3     | 2,045  |  18,404 |  type i          |
| VH266015 | 0.963 |         1 |     4     | 1,927  |  17,340 |  type ii         |
| VH266510 | 0.933 |         1 |     3     | 4,502  |  40,518 |  type i          |
| VH269384 | 0.970 |         1 |     4     | 1,966  |  17,695 |  type ii         |
| VH271613 | 0.977 |         1 |     4     | 4,175  |  37,579 |  type ii         |
| VH302907 | 0.980 |         1 |     2     | 4,357  |  39,210 |  type i          |
| VH304954 | 0.985 |         1 |     3     | 2,790  |  25,107 |  type i          |
| VH507804 | 0.991 |         1 |     4     | 2,000  |  17,995 |  type ii         |
| VH525628 | 0.957 |         1 |     3     | 1,946  |  17,509 |  type i          |
